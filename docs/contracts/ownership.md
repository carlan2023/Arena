...
Status: frozen. Owner: Supervisor. Changes go through the supervisor and are logged in docs/decisions.md.
...
| Server | M2.1, M2.2, M2.4 to M2.8 | server (except server/auth, server/wallet and server/test/integration), packages/arena_protocol, docker-compose.yml, server/Dockerfile, server/.env.example |
...
| apps/mobile | arena | ludo_engine, ludo_bots, arena_protocol by path, flutter_riverpod, go_router |
